		...
		<view class="index-content u-p-30 u-skeleton" :style="[indexStyle]">
			...
			<view class="u-m-t-30 bg-white u-p-15 u-flex grid">
				...
				<view class="u-m-l-15" v-if="vuex_store.is_more == true" @click="goPage('/pages/store/cutlist')">
					<u-icon color="#838383" name="/static/image/switch.png" size="60"></u-icon>
				</view>
			</view>
...
export default {
	...
	computed: {
		notice() {
			let arr = [];
			console.log(this,'挂载的数据');
			console.log(this.vuex_config.navigate,'获取的数据类型');
			// console.log(this.vuex_config.notice,'这是获取的公告数据');
			if (this.vuex_config.notice) {
				this.vuex_config.notice.map(item => {
					arr.push(item.title);
				});
			}
			console.log(arr,'获取的路径');
			return arr;
		},
		indexStyle() {
			let style = {};
