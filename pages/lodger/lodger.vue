<template>
	<view>
		...
		<view class="u-flex u-row-center fa-emmpty top-15" v-if="!list.length">
			...
			<view class="u-tips-color">暂无住客信息~</view>
		</view>
		<!-- 加载更多 -->
	...
	methods: {
		...
		lodgerList() {
			this.$api.lodgerList({ page: this.page }).then(res => {
				...
				if (res.code == 1) {
					let vuex_lodgerids = []
					if(this.vuex_storeType==3&&this.vuex_lodgerids.length!=0){
						vuex_lodgerids = this.vuex_lodgerids.splice(0,1)
					}
					if(this.vuex_storeType!=3){
