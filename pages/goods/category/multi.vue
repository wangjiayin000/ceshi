<template>
	<view class="u-wrap" :style="[height]">
		<view class="u-menu-wrap">
			...
			<scroll-view :scroll-top="scrollRightTop" scroll-y scroll-with-animation v-if="!is_empty" class="right-box"
				@scroll="rightScroll">
				<view class="page-view">
					<view class="class-item" :id="'item' + index" v-for="(item, index) in category" :key="index">
						...
						<view class="item-container">
							<view class="thumb-box" v-for="(item1, index1) in item.childlist" :key="index1"
	...
	export default {
		...
		methods: {
			...
			async leftMenuStatus(index) {
				if (this.category[index].childlist.length < 5) {
					return;
				}
				this.current = index;
				// 如果为0，意味着尚未初始化
			...
			getCategory() {
				this.$api.getCategory().then(res => {
					if (res.code == 1) {
						...
						this.is_empty = this.category.length > 0 ? false : true;
					} else {
						this.is_empty = true;
					}
				});
