	...
	<view class="">
		...
		<view class="u-skeleton">
				...
				<u-form ref="uForm">
					...
					<u-form-item :label-position="labelPosition" label="住客信息" prop="code" label-width="130">
						...
						<u-button
							...
							@click="
								is_update = true;
								goPage('/pages/lodger/lodger?livenums='+info.livenums+'&storeType='+info.store.type);
							"
						>
							添加/选择
						</u-button>
			...
			<view class="bg-white u-m-30 u-p-30 u-skeleton-rect">
				<view class="u-flex u-col-center u-row-between u-p-b-20">
				...
				<view class="u-flex u-col-center u-row-between u-p-b-20" v-if="child_price">
					<view class="u-font-30 u-tips-color">儿童餐饮费用</view>
					<view class="prices u-text-right">
						<text class="price text-weight u-font-35">￥{{ child_price }}</text>
						<!-- <text class="u-tips-color u-font-24">共 {{ calendar.day || 0 }} 晚</text> -->
					</view>
				</view>
				<view class="u-flex u-col-center u-row-between u-p-b-20" v-if="deposit!='0.00'">
					<view class="u-font-30 u-tips-color">房间定金</view>
					<view class="prices u-text-right">
						<text class="price text-weight u-font-35">￥{{ deposit }}</text>
						<!-- <text class="u-tips-color u-font-24">共 {{ calendar.day || 0 }} 晚</text> -->
					</view>
				</view>
...
export default {
	...
	computed:{		
		houseTotalPrice(){
			let price = parseFloat(this.calendar.amount ? this.calendar.amount : this.info.price);
			...
			this.$api
				...
				.then(res => {
					if (res.code == 1) {	
						...
						this.$u.vuex('vuex_storeType', this.info.store.type);
						if(this.info.store.type==3&&this.vuex_lodgerids.length!=0){
							vuex_lodgerids = this.vuex_lodgerids.splice(0,1)
						}
						if(this.info.store.type!=3){
							vuex_lodgerids = this.vuex_lodgerids
						}
						...
						if(this.info.store.type==3&&vuex_lodgerids.length!=0){
							this.queryPrice(vuex_lodgerids[0])
						}
						this.loading = false;
					} else {
		...
		queryPrice(e){
			if(this.info.store.type==3){
				this.$api
					...
					.then(res=>{
						this.child_price = res.data.child_price&&res.data.child_price.toFixed(2)
						this.deposit = res.data.details.deposit
					})
			}
		},
		submit() {
